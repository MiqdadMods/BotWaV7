		...
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					...
					ppimg = 'https://a.top4top.io/p_1846qbbg40.jpg'
				}
				teks = `[ NEW MEMBER IN GROUP _*${mdata.subject}*_ ] \n*_____________*\n@${num.split('@')[0]}\nSELAMAT BERGABUNG\n *_____________*\nMoga Betah ya Di Grup Ini Ya Bep`
				...
				teks = `hey kak @${num.split('@')[0]} Kalo Mau Balik Miqdad Titip Gorengan yee:v`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MIQDAD MODS*',
			...
			const mod = [ownerNumber,"6285326364468@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["6285326364468@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["6285326364468@s.whatsapp.net"]//ubah nomor lo
			const premium = ["6285326364468@s.whatsapp.net","6285326364468@s.whatsapp.net"]
			...
			const errorurl2 = 'https://a.top4top.io/p_1846qbbg40.jpg'
		...
		case 'iri':
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER BOT  MIQDAD MOD👑* 」\n\`\`\``
					no = 0
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*MIQDAD MODS BOT\n*PULSA6285326364468 :* _08165466368_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER BOT  MIQDAD MODS👑* 」\n\`\`\``
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
 ...
 if (isBanned) return reply(mess.only.benned)    
                ...
                client.sendMessage(hasil, image, mek, '\`\`\`MIQDADMODSJOKES:V\`\`\`')
				break
					case 'kata':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('TEXT NYA MANA BANG??:)')
					jhf = body.slice(10)
					reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
                   ...
                   break
                case 'indohot [DELETE BY MIQDAD MODS]':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey={BELI APIKEY BIAR WORK DI 085326364468}`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/infomobil?merk=${ige}&apikey={BELI APIKEY BIAR WORK DI 085326364468}`, {method: 'get'})
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`PERINTAH DI TERIMA , BERHASIL MENGUBAH GRUP , MEMBER BISA KEMBALI BERSENANG2`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`PERINTAH DI TERIMA , HANYA ADMIN YG DAPAT MENGIRIM PESAN`)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  MIQDAD MODS BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
